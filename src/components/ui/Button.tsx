...
function variantClasses(variant: Variant): string {
  switch (variant) {
    case "primary":
      return cn(
        "border-2 border-sand bg-shell text-ink shadow-[var(--shadow-level-1)]",
        "before:bg-olive hover:text-paper",
      );
    case "blue":
      return cn(
        "border-2 border-sand bg-paper text-ink shadow-[var(--shadow-level-1)]",
    ...
    case "secondary":
      return cn(
        ...
        "before:bg-mist hover:text-ink",
      );
    case "ghost":
      return cn("border border-transparent bg-transparent text-ink", "before:bg-mist");
  }
}
...
const Arrow = ({ size }: { size: Size }) => (
  <svg
    ...
    className={cn(
      ...
      "rotate-45 rounded-full border border-soft-ink p-1.5 transition duration-300 ease-linear",
